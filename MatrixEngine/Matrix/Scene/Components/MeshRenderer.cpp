...
void Scene::Components::MeshRenderer::LoadShader(std::string vertex, std::string fragment, std::string geometry, std::string tessellation1, std::string tessellation2)
{
	...
	if(geometry != "")shader_geom = std::make_shared<Shaders::ShaderSource>();
	if(tessellation1 != "")shader_tess1 = std::make_shared<Shaders::ShaderSource>();
	if(tessellation2 != "")shader_tess2 = std::make_shared<Shaders::ShaderSource>();
